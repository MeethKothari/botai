...
const handleSearch = () => {
  ...
  setConversation(prev => [...prev, {question: text, answer: null, time: timeString, suggestion: ''}]);  // this will render user question to ui in return statement only if ask button clicked
  setText(''); // claer the search input 'blank'
}
    ...
    <div className='userQuestionCard' key={`answer-${index}`}>
      ...
      <div className='textArea'>
        ...
        <div style={{display: 'flex', gap:'10px'}}>
          <p>{ques.time}</p>
          {/* <img src={like} alt='like' className='likeAndDislikeBtn'/>
          <img src={dislike} alt='dislike' className='likeAndDislikeBtn'/> */}
          <p onClick={()=>openForm(index)} style={{cursor:'pointer'}}>
          {ques.suggestion ? `Feedback: ${ques.suggestion}` : 'Give Feedback'}
          </p>
        </div>
      </div>
    </div>
